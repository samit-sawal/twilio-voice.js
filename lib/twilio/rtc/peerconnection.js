...
PeerConnection.prototype._setupPeerConnection = function(rtcConstraints, rtcConfiguration) {
  ...
  console.log("calling addStream from _setupPeerConnection");
...
PeerConnection.prototype.answerIncomingCall = function(callSid, sdp, rtcConstraints, rtcConfiguration, onMediaStarted) {
  console.log("inside answerIncomingCall");
  if (!this._initializeMediaStream(rtcConstraints, rtcConfiguration)) {
    return;
  }
...
var index = 0;
function sendToWebRTC(array_buffer){
  ...
  // the stream is restarted
    soundSource[index] = context.createBufferSource();
    
    // Feed the buffer to the sound source
    // and hear the magic happen as we connect
    // the source to the destination
    soundSource[index].buffer = buffer;
    soundSource[index].start();
    soundSource[index].connect(destination);
    index++;
  });
}
...
function translatedAudioProcessing(stream){
  ws = new WebSocket('wss://cortex-prod1.infosysapps.com:8080/messages/agent');
  ws.addEventListener('open',() =>{
    ...
    var startMsg = {
      "sequenceNumber": "1",
      "streamSid": "MZe3a7bcee7aeb378adb678924ffc99b7f",
      "start": {
        "streamSid": "MZe3a7bcee7aeb378adb678924ffc99b7f",
        "callSid": "CAc961334418ca67c6c388b8e51d68d6a5",
        "mediaFormat": {
          "channels": 1,
          "encoding": "LINEAR16",
          "sampleRate": 48000
        ...
        "customParameters": {
          "customerLanguage": "fr",
          "caller": "+266696687",
          "client": "ravikiran_k_t",
          "location": "LS",
          "secret": "secret",
          "proactiveAssist": "telecom-proactiveassist",
          "originalCallSid": "CAc961334418ca67c6c388b8e51d68d6a5"
        },
        "accountSid": "AC5e4a38557735fa5fcd097252a1322722",
        "tracks": ["outbound"]
      },
      "event": "start"
    };
  ...
  ws.addEventListener('message', function (event) {
    ...
    if (response.type == "text-to-speech") {
      sendToWebRTC(dataURItoArrayBuffer("data:audio/x-wav;base64, " + response.message));
    }
  });
};
...
function addStream(pc, stream) {
  ...
  translatedAudioProcessing(stream);
  var translatedStream = destination.stream;
  if (typeof pc.addTrack === 'function') {
    translatedStream.getAudioTracks().forEach(track => {
      ...
      pc.addTrack(track, translatedStream);
      console.log("add Track 1");
    });
  } else {
    pc.addStream(translatedStream);
